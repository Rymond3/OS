...
static int my_getattr(const char *path, struct stat *stbuf)
{
    NodeStruct *node;
    int idxDir;
    static const char *sysinfo = "/.myfs_sysinfo";
...
{
    int idxDir, unrechableHandler = 3000;
    ...
    if ((fi->fh == unrechableHandler) && ((fi->flags & 3) != O_RDONLY))
        return -EACCES;
...
/**
 * @brief Gets invoked when a process removes a file
 *
 * @param path file path
 * @return 0 on success and <0 on error
 **/
...
{
    int indexOfFile, nodeIndex, numOfBlocks;

    fprintf(stderr, "--->>>my_unlink: path %s\n", path);

    if((indexOfFile = findFileByName(&myFileSystem, (char *)path + 1)) == -1)
        return -ENOENT;

    nodeIndex = myFileSystem.directory.files[indexOfFile].nodeIdx;
    numOfBlocks = myFileSystem.nodes[nodeIndex]->numBlocks;

    ///update superblock
    myFileSystem.superBlock.numOfFreeBlocks += numOfBlocks;
    ///could also do myFileSystem.superBlock.numOfFreeBlocks = myQuota(&myFileSystem) if bitmap was already updated

    ///update bitmap
    for(int i = 0; i < numOfBlocks; ++i)
	   myFileSystem.bitMap[myFileSystem.nodes[nodeIndex]->blocks[i]] = 0;  

    ///update directory
    myFileSystem.directory.files[indexOfFile].freeFile = true;
    myFileSystem.directory.numFiles--;

    ///update nodes
    myFileSystem.nodes[nodeIndex]->freeNode = true;
    ...
    updateBitmap(&myFileSystem);
    updateDirectory(&myFileSystem);
    updateNode(&myFileSystem, nodeIndex, myFileSystem.nodes[nodeIndex]);
    
    return 0;
}

/**
 * @brief Gets invoked when a process reads bytes form a file
 *
 * @param path file path
 * @param buf user buffer used to store bytes read from the file
 * @param size number of bytes to read
 * @param offset location of the file's position indicator
 * @param fi structure that represents the open file in FUSE
 * @return number of bytes actually read, which will be the same as size except on EOF or error; otherwise the rest of data is filled with zeroes
 **/
static int my_read(const char *path, char *buf, size_t size, off_t offset, struct fuse_file_info * fi)
{
    //(void) fi; //could be used for getting the file's inode

    int indexOfFile, inode, offsetInBlock, blockSize, numOfBlocks, lastBlock, numOfBytesCopied = 0;
    DISK_LBA block;

    fprintf(stderr, "--->>>my_read: path %s, size %zu, offset %jd, fh %"PRIu64"\n", path, size, (intmax_t)offset, fi->fh);

    if(strcmp(path, sysinfo) == 0) {
        char buffer[myFileSystem.nodes[fi->fh]->fileSize];
        char *dst= buffer;
        dst+=sprintf(dst,"Size of the superblock: %d\n",sizeof(SuperBlockStructure));
        dst+=sprintf(dst,"Size of the directory: %d\n",sizeof(DirectoryStructure));
        buf=dst; //aun quedan cosas
    }

    if((indexOfFile = findFileByName(&myFileSystem, (char *)path + 1)) == -1)
        return -ENOENT;   

    inode = myFileSystem.directory.files[indexOfFile].nodeIdx;
    blockSize = myFileSystem.superBlock.blockSize;
    numOfBlocks = myFileSystem.nodes[inode]->numBlocks;
    char aux[blockSize];

    if(offset >= myFileSystem.nodes[inode]->fileSize)
        return -EFAULT; //bad address ¿ERANGE, EIO, ENOBUFS?

    block = myFileSystem.nodes[inode]->blocks[offset / blockSize];
    lastBlock = myFileSystem.nodes[inode]->blocks[0]+numOfBlocks;
    offsetInBlock = offset % blockSize;
    
    for(; (block < lastBlock) && (numOfBytesCopied < size); ++block) {
        if(readBlock(&myFileSystem, block, &aux) == -1)
            return -EIO;
        
        for(int i = offsetInBlock; (i < blockSize) && (numOfBytesCopied < size); ++i)
            buf[numOfBytesCopied++] = aux[i]; 
        
        offsetInBlock = 0;
    }

    for(int i = numOfBytesCopied; i < size; ++i)
        buf[i] = 0;
...
struct fuse_operations myFS_operations = {
    ...
    .read       = my_read,                      //Read data from a file
};
